...
class Viscosity(nn.Module):
    ...
    def forward(self, X, training=True):
        ...
        out = x1 * (out1[:, 0] + out1[:, 1] / T + out1[:, 2] / (T - self.beta) + out1[:, 3] * torch.log(
            ...
            x2 * mw2)) + \
              x1 * x2 * self.inter_para.reshape(-1, )
        return out
...
early_stopping = EarlyStopping(patience=1000, model_type='TSTiNet-variables')
...
model.load_state_dict(torch.load('../outputs/model_parameters/TSTiNET-variables.pt'))
...
outputs = model(X_train)
train_para_results = {'beta': model.beta.tolist(),
                      'inter_para': model.inter_para.tolist()}

save_outputs([test_results, val_results, train_results, para_results, loss_trace, train_para_results],
             ['test_results',
              'val_results',
              'train_results',
              'para_results',
              'loss_results',
              'train_para_results'], suffix=file_path)
